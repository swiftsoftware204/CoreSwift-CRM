...
pub async fn list(State(s): State<AppState>, Extension(c): Extension<Claims>) -> ApiResult<impl IntoResponse> {
    ...
    let tenants = if c.role == "agency_admin" {
        sqlx::query_as::<_, Tenant>("SELECT * FROM tenants WHERE is_active=true ORDER BY name").fetch_all(&s.db).await?
    } else {
        sqlx::query_as::<_, Tenant>("SELECT * FROM tenants WHERE id=$1 AND is_active=true").bind(t).fetch_all(&s.db).await?
    };
    Ok(Json(json!({"tenants": tenants})))
}

pub async fn create(State(s): State<AppState>, Extension(c): Extension<Claims>, Json(r): Json<CreateTenantRequest>) -> ApiResult<impl IntoResponse> {
    ...
    let tenant = sqlx::query_as::<_, Tenant>("INSERT INTO tenants(id,name,slug,logo_url,primary_color,accent_color,custom_domain) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *")
...
pub async fn get(State(s): State<AppState>, Extension(c): Extension<Claims>, Path(id): Path<Uuid>) -> ApiResult<impl IntoResponse> {
    let t = Uuid::parse_str(&c.tid).map_err(|_| AppError::Unauthorized)?;
    if c.role != "agency_admin" && t != id { return Err(AppError::Forbidden); }
    Ok(Json(json!(sqlx::query_as::<_, Tenant>("SELECT * FROM tenants WHERE id=$1").bind(id).fetch_optional(&s.db).await?.ok_or(AppError::NotFound(format!("Tenant {id} not found")))?)))
}

pub async fn update(State(s): State<AppState>, Extension(c): Extension<Claims>, Path(id): Path<Uuid>, Json(r): Json<UpdateTenantRequest>) -> ApiResult<impl IntoResponse> {
    let t = Uuid::parse_str(&c.tid).map_err(|_| AppError::Unauthorized)?;
    if c.role != "agency_admin" && t != id { return Err(AppError::Forbidden); }
    Ok(Json(json!(sqlx::query_as::<_, Tenant>("UPDATE tenants SET name=COALESCE($1,name), slug=COALESCE($2,slug), logo_url=COALESCE($3,logo_url), primary_color=COALESCE($4,primary_color), accent_color=COALESCE($5,accent_color), custom_domain=COALESCE($6,custom_domain), is_active=COALESCE($7,is_active), updated_at=NOW() WHERE id=$8 RETURNING *")
        .bind(&r.name).bind(&r.slug).bind(&r.logo_url).bind(&r.primary_color).bind(&r.accent_color).bind(&r.custom_domain).bind(r.is_active).bind(id).fetch_optional(&s.db).await?.ok_or(AppError::NotFound(format!("Tenant {id} not found")))?)))
}
...
pub async fn delete(State(s): State<AppState>, Extension(c): Extension<Claims>, Path(id): Path<Uuid>) -> ApiResult<impl IntoResponse> {
    ...
    if r.rows_affected() == 0 { return Err(AppError::NotFound(format!("Tenant {id} not found"))); }
...
pub async fn get_settings(State(s): State<AppState>, Extension(c): Extension<Claims>, Path(id): Path<Uuid>) -> ApiResult<impl IntoResponse> {
    let t = Uuid::parse_str(&c.tid).map_err(|_| AppError::Unauthorized)?;
    if c.role != "agency_admin" && t != id { return Err(AppError::Forbidden); }
    let settings = sqlx::query_scalar::<_, Option<serde_json::Value>>("SELECT settings FROM tenants WHERE id=$1").bind(id).fetch_optional(&s.db).await?.ok_or(AppError::NotFound(format!("Tenant {id} not found")))?;
