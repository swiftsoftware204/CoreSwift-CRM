...
pub async fn get_score(State(s): State<AppState>, Extension(c): Extension<Claims>, Path(cid): Path<Uuid>) -> ApiResult<impl IntoResponse> {
    ...
    Ok(Json(json!(sqlx::query_as::<_,Score>("SELECT * FROM scores WHERE tenant_id=$1 AND contact_id=$2").bind(t).bind(cid).fetch_optional(&s.db).await?.ok_or(AppError::NotFound(format!("Score for contact {cid} not found")))?)))
}
