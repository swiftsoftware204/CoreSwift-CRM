...
pub async fn evaluate_tag_triggers(db: &PgPool, tenant_id: Uuid, entity_type: &str, entity_id: Uuid, tag_id: Uuid, trigger_type: &str) -> Result<(), AppError> {
    let rules = sqlx::query_as::<_, AutomationRule>("SELECT * FROM automation_rules WHERE tenant_id=$1 AND trigger_type=$2::trigger_type AND is_enabled=true")
